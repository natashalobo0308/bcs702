...
int main(){
    int n;
    printf("Enter The Upper Limit(n) to find prime number:");
    scanf("%d",&n);
    if(n<2){
    ...
    #pragma omp_parallel for reduction(+:parallel_prime_count)schedule(dynamic)
    for(int i=1;i<=n;i++){
        if(is_prime(i))parallel_prime_count++;
    }
    double time_par=omp_get_wtime()-start_time;
    printf("\n Parallel :Found %d primes in %f seconds\n",parallel_prime_count,time_par);
